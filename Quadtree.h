...
const int power[21]={1,2,4,8,16,32,64,128,256,512,1024,2048,4096,8192,16384,32768,65536,131072,262144,524288,1048576};
                     // 1*1(s = 0), 2*2(s = 1), 4*4(s = 2), ....
class quad_tree 
{
    public:
    int data, s;           // data : 1/0 for Binary image
   ...
   // get pixel value of a particular coordinate
    ...
    // Intersection(Either White(0) is White)
    ...
    long long int m(){
        if(q1!=NULL) 
          return this->q1->m()+this->q2->m()+this->q3->m()+this->q4->m();
        if(this->data==1) 
        {
            long long int x = power[this->s];                            
            return x*x; 
